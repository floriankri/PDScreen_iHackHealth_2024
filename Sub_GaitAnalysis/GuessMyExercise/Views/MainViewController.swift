...
extension MainViewController {
    ...
    private func updateUILabelsWithPrediction(_ prediction: ActionPrediction) {
        ...
        if confidenceString == "Observing..." || confidenceString == "No Person" {
            DispatchQueue.main.async { self.confidenceLabel.text = "Observing..." }
        }
    }
        ...
        let frameWithPosesRendering = poseRenderer.image { rendererContext in
            ...
            for pose in poses {
                ...
                DispatchQueue.main.async {
                    let formattedString = String(format: "%.3f", pose.landmarkDistance())
                    let fullString = "Current measurement: \(formattedString)"
                    self.confidenceLabel.text = fullString
                }
                }
            }
